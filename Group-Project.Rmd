...
car::vif(salaries_model_back_aic) #Nothing too bad, perhaps level is high
...
aic_model_trans = lm(log(totalyearlycompensation) ~ (company + yearsofexperience) ^ 2 + title + gender + Race + Education + level, data = trn)

#Remove influential data from the model
aic_model_fix = lm(log(totalyearlycompensation) ~ (company + yearsofexperience) ^ 2 + title + gender + Race + Education + level, data = salaries, subset = cooks.distance(aic_model_trans) < 4 / length(cooks.distance(aic_model_trans)))
...
for(i in 1:num_sims){
  sim_fit = lm(log(totalyearlycompensation) ~ (company + yearsofexperience + title) ^ 2 + gender + Race + Education + level, data = trn)
