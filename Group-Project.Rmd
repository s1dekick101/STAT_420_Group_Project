...
#Prediction of how much more males will be paid more than their female counterparts
...
##### 1. General Trends
- **Google vs. Amazon**: Google tends to offer higher salaries than Amazon (Graph 1).
- **Impact of Levels on Salary**: Salaries increase as levels rise (Graph 2).
- **Tech vs. Non-tech Job Titles**: Tech roles command higher salaries compared to non-tech roles (Graph 3).
...
- **Education**: Both companies predict the highest starting entry-level salaries for those with Ph.Ds; however, the second-highest predicted salary is for those with only 'some college' experience (Graph 6).
...
Both points mentioned above, combined with our real-world knowledge of Amazon and Google salaries, brings into question some of our model inferences. For example, two individuals with the same parameters, one at Amazon and one at Google, have drastically different L3 predicted salaries (Amazon: \$`r round(amazon_l3, 0)`, Google: $`r round(google_l3, 0)`.) Similarly, we would also question that individuals with 'Some College' educations would earn more on average than those with 'Master's Degrees'. Ultimately, although we were able to produce the 'best' model, we were still constrained by the limitations of the data itself.
